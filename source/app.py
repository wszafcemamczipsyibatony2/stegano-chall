...
app.add_url_rule("/temp/<name>", endpoint="check_exif_tool", build_only=True)
...
def upload_file():
    if request.method == "POST":
        ...
        if file:
            ...
            return redirect(TEAM_LINK + url_for("check_exif_tool", name=fname))

    return f"""
	<!doctype html>
	<title>Stegano solver22!</title>


	<h1>Stegano solver2!</h1>
	...
	<a href="{TEAM_LINK}/temp/bG9nby5wbmc=">Click!</a>
	"""
...
def check_exif_tool(name):
    ...
    for line in output:
        html += f"<li>{line}</li>\n"
    html += "</ul>"
    html += f"<a href={TEAM_LINK}/>Home</a>"
    return html
